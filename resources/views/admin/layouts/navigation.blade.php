...
<div class="h-16 bg-white shadow-sm pl-80 pr-8 fixed w-full top-0 left-0 flex items-center">
    ...
    <div class="ml-auto flex intems-center">
        ...
        <div class="ml-4 relative">
            ...
            <div id="dropdown-item" class="hidden absolute z-50 mt-2 rounded-md shadow-lg w-48 right-0 py-1 bg-white">
                <div class="px-4 py-2 text-us text-gray-400">Manage Account</div>
                <a href="{{ route('admin.profile.edit') }}" class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 transition">Profile</a>
                ...
                <form method="POST" action="{{ route('admin.logout') }}">
                    @csrf
                <a href="{{ route('admin.logout') }}" onclick="event.preventDefault();this.closest('form').submit();" class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 transition">Logout</a>
                </form>
            </div>
        </div>
    </div>
